...
use reqwest::{
    header::{
        CONTENT_LENGTH, USER_AGENT, COOKIE
    },
};
...
//     async_get().await?; // Call the async function and await its completion
...
fn blocking_get() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let res = client.get("https://api.github.com/users/bvenkatr")
...
async fn async_get() -> Result<(), reqwest::Error> {
    let client = reqwest::Client::new();
    let res = client
        .get("https://api.github.com/users/bvenkatr")
        .header(USER_AGENT, "My Rust Program 1.0")
        .send()
        .await?; // Use `await` here to wait for the completion of the future

    if res.status().is_success() {
        let body = res.text().await?; // Use `await` to get the response body
        println!("body = {:?}", body);
    } else {
